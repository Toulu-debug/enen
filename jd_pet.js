...
function runTimes() {
  return new Promise((resolve, reject) => {
    $.get({
      url: `http://sharecodepool.cnmb.pw/api/runTimes0917?activityId=pet&sharecode=${$.petInfo.shareCode}`
    }, (err, resp, data) => {
      if (err) {
...
function readShareCode() {
  return new Promise(async resolve => {
    $.get({url: `http://sharecodepool.cnmb.pw/api/pet/${randomCount}`, 'timeout': 10000}, (err, resp, data) => {
      try {
        if (err) {
