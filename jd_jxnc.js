  ...
  if (startInfo) {
    ...
    if (startInfo.target <= startInfo.score) { // 水滴已满
      if (startInfo.activestatus === 2) { // 成熟未收取
        ...
        // 水果成熟单独通知
        if (notifyLevel >= 1) {
          let sendContent = `【京东账号${$.index}】${$.nickName}\n`;
          sendContent += `【水果名称】${startInfo.prizename}\n`
          sendContent += `【成熟】水果已成熟请及时收取，activestatus：${startInfo.activestatus}\n`;
          sendContent += `请去京喜APP或京喜微信小程序收取\n`;
          $.msg($.name, `${$.name} - 账号${$.index} - ${$.nickName}水果已收取`, 'sendContent', option);
          
          if ($.isNode()) {
            await notify.sendNotify(`${$.name} - 账号${$.index} - ${$.nickName}水果已收取`, sendContent);
          }
        }
